...
function Crear_Formulario_Queja(id) {
    ...
    var cuerpoIzquierda = CreaInputs_Con_Labeldisabled('idqueja', 'idqueja', '', 'text', 'ID:', 'textfield', 'mes')
        + `<button type="button" class="" style="border:hidden; background:none;" title="Cédula de Información recabada por la DQOT" onclick="GeneraDocumento_pdf('pilin','IDQOT',${id})"> <span aria-hidden="true"><i class="fa fa-file-pdf-o" style="color: red;"></i></span> </button>`
        + CreaBR()
...
function Crear_Formulario_QuejaEdit(id, tipo) {
    ...
    var cuerpoDerecha = Crea_Label('textfield8', 'textfield8', '', Requeridos() + 'Lugar de los hechos. Municipio y Estado: ')
        ...
        + checkbox('Validar info. DQOT', `confi_peticiona${tipo}`, '', 'disabled', 'pulsacionrellow') + `<span id="cont_pet${tipo}" style="font-size: 12px;">0/0</span><span id ="cont_pet_desc${tipo}"style="font-size: 12px;"> peticionarios(a) confirmados</span>`
        + CreaBR()
...
function obtenerDQOTModifica(idqueja, fecRecep, tipo, expedienten) {
    ...
    if (version !== "") {
        ...
        ajaxDQOT.done(function (response) {
            ...
            if (response.informarcionC.fecha_registro != null) {
                //JM
                let fechaStr = response.informarcionC.fecha_registro.split(' ')[0]; // "29/10/2025"

                // Convertir DD/MM/YYYY → YYYY-MM-DD
                let [dia, mes, anio] = fechaStr.split('/');
                let formatoValido = `${anio}-${mes}-${dia}`; // "2025-10-29"

                // Asignar al input tipo date
                document.getElementById(`Fecha_Registro${tipo}`).value = formatoValido;
            }
            if (fecRecep != null) {
                let formatoValidoFTVG = normalizaFecha(fecRecep);
                document.getElementById(`Fecha_TurnoVG${tipo}`).value = formatoValidoFTVG;
                //let Fecha_TurnoVGjm = fecRecep.split(' ')[0]; 

                //// Convertir DD/MM/YYYY → YYYY-MM-DD
                //let [dia, mes, anio] = Fecha_TurnoVGjm.split('/');
                //let formatoValidoFTVG = `${anio}-${mes}-${dia}`; // "2025-10-29"
        ...
        $(`#confi_lughec${tipo}`).change(function () {
            if (tipo === 'E') {
                if ($(this).is(':checked')) {
                    // Se marca confi_lughecE
                    $('#icomuniE').prop('hidden', true);       // Oculta icomuniE
                    $('#municipioquejaE').prop('disabled', true); // Deshabilita el municipio
                    $(`#confi_lughec${tipo}`).removeClass('pulsacionrellow');
                    confirmdatos($('#idquejaE').val(), '', '1', '');
                } else {
                    // Se desmarca confi_lughecE → volver todo a normal
                    $('#icomuniE').prop('hidden', false);     // Mostrar icomuniE
                   /* $('#icomuniE').prop('checked', false);    // Desmarcar icomuniE si estaba marcado*/
                    $('#municipioquejaE').prop('disabled', false); // Habilitar municipio
            ...
            <div id='Divpequenios'>
                <div class="dummy dummy-text">
                    <p>
                        ...
                        <!-- 🔹 CAMBIO: se agregó botón lupa también en este caso -->
...
function formActacircunstanciada2c() {
    ...
    var cuerpo =
        ...
        + Crea_Label('textfield8', 'textfield8', '', 'de la Comisión de Derechos Humanos del Estado de Puebla, con la fe pública que me confiere el artículo 21 y 31 de la Ley de de este Organismo, así como 7, 8, 9, 48, 59, 60, 61, 64 y 66 de su Reglamento Interno, publicados en el Periódico Oficial del Estado, respectivamente')
...
function formEscritoInicial2(action, id) {
    ...
    var cuerpo = '<div class="text-right">'
        ...
        + '</div>'
        + Crea_Parrafos('parrafo0', 'parrafo0', 'col-md-3 parrafo', 'MTRA. ROSA ISELA SANCHEZ SOYA</br>PRESIDENTA DE LA COMISIÓN DE DERECHOS HUMANOS DEL ESTADO DE PUEBLA', 'style ="text-align: left;font-weight: bold;"')
...
function GeneraDocumento_pdf(nombreDocumento, tipo, idq) {
    ...
    if (nombreDocumento == 'undefined' || nombreDocumento == '') {
        ...
        if (tipo === 'AC') {
            window.open(
                '/Expediente/Acta_Circunstanciada?idactac=' + idq,
                '_blank'
            );
        } else if (tipo == 'EI') {
            window.open(
                '/Expediente/EscritoInicial?idEscrito=' + idq,
                '_blank'
            );
        }
        else if (tipo == 'IDQOT') {
            ...
            window.open(
                '/Expediente/ConsutlarArchivoCalificación?fileName='
